...
class DrmDevice {
 public:
  ...
  bool Test(int64_t display, const std::shared_ptr<DrmFramebuffer>& fb,
            int acquire_fence);
  bool TestConfiguration(int64_t display);
  android::base::unique_fd Present(
 ...
 private:
  ...
  bool AtomicCommit(DrmDisplay* display,
                    const std::shared_ptr<DrmFramebuffer>& fb,
                    int acquire_fence, bool test_only,
                    android::base::unique_fd* out_fence);
  uint32_t GetPropertyId(uint32_t object_id, uint32_t object_type,
                         const char* name, uint64_t* value = nullptr) const;
  bool AddProperty(drmModeAtomicReq* request, uint32_t object_id,
