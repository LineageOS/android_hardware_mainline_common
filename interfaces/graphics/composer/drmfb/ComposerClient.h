...
class ComposerClient : public BnComposerClient {
 public:
  ...
  ndk::ScopedAStatus createLayer(int64_t display, int32_t slot_count,
                                  int64_t* layer) override;
  ndk::ScopedAStatus createVirtualDisplay(
      int32_t width, int32_t height, common::PixelFormat format_hint,
      int32_t output_slot_count, VirtualDisplay* display) override;
  ...
  ndk::ScopedAStatus getColorModes(int64_t display,
                                   std::vector<ColorMode>* modes) override;
  ...
  ndk::ScopedAStatus getDisplayIdentificationData(
      int64_t display, DisplayIdentification* identification) override;
  ...
  ndk::ScopedAStatus getReadbackBufferAttributes(
      int64_t display, ReadbackBufferAttributes* attributes) override;
  ndk::ScopedAStatus getReadbackBufferFence(
      int64_t display, ndk::ScopedFileDescriptor* fence) override;
  ...
  ndk::ScopedAStatus setHdrConversionStrategy(
      const common::HdrConversionStrategy& strategy, common::Hdr* hdr) override;
 ...
 private:
  ...
  bool SetClientTargetLocked(int64_t display, const ClientTarget& target,
                             int32_t* error);
  bool ApplyLayerLocked(int64_t display, const LayerCommand& command,
                        int32_t* error);
  void AddError(size_t index, int32_t error,
                std::vector<CommandResultPayload>* results);
  void ValidateLocked(int64_t display,
                      std::vector<CommandResultPayload>* results);
  bool PresentLocked(int64_t display,
                     std::vector<CommandResultPayload>* results,
                     int32_t* error);
  void HotplugLoop();
  void VblankLoop();
