...
std::string ModeKey(uint32_t connector_id, const drmModeModeInfo& mode) {
  ...
  key << connector_id << ':' << mode.clock << ':' << mode.hdisplay << ':'
      ...
      << mode.flags << ':' << mode.vrefresh;
  return key.str();
}

bool IsUsableEdid(const std::vector<uint8_t>& edid) {
  static constexpr uint8_t kHeader[] = {0x00, 0xff, 0xff, 0xff,
                                         0xff, 0xff, 0xff, 0x00};
...
bool DrmDevice::Init(const std::string& path) {
  ...
  modifiers_supported_ =
      drmGetCap(fd_.get(), DRM_CAP_ADDFB2_MODIFIERS, &modifiers) == 0 &&
      modifiers != 0;
  Rescan();
  ALOGI("Opened %s with %zu connector records; modifiers=%d", path.c_str(),
        displays_.size(), modifiers_supported_);
  return true;
}
...
std::vector<DrmDevice::HotplugChange> DrmDevice::Rescan() {
  ...
  for (uint32_t connector_id : connectors) {
    ...
    if (!FindPipeline(drmModeGetConnector(fd_.get(), connector_id), &candidate,
                      used_crtcs, used_planes) ||
...
bool DrmDevice::DiscoverConnector(uint32_t connector_id, DrmDisplay* display) {
  ...
  display->name = StringPrintf("%s-%u", drmModeGetConnectorTypeName(
  ...
  if (display->connected) {
    for (int i = 0; i < connector->count_modes; ++i) {
      ...
      const bool duplicate = std::any_of(
          display->configs.begin(), display->configs.end(),
      ...
      config.id = it->second;
      config.group = (static_cast<int32_t>(config.mode.hdisplay) << 16) |
                     config.mode.vdisplay;
  ...
  for (int e = 0; e < connector->count_encoders && !found; ++e) {
    ...
    for (int c = 0; c < resources->count_crtcs && !found; ++c) {
      ...
      if ((encoder->possible_crtcs & (1U << c)) == 0 ||
          std::find(used_crtcs.begin(), used_crtcs.end(), crtc) !=
              used_crtcs.end()) continue;
...
uint32_t DrmDevice::GetPropertyId(uint32_t object_id, uint32_t object_type,
  ...
  for (uint32_t i = 0; i < properties->count_props; ++i) {
    drmModePropertyRes* property =
        drmModeGetProperty(fd_.get(), properties->props[i]);
...
bool DrmDevice::DiscoverProperties(DrmDisplay* d) {
  ...
  p.connector_crtc_id = GetPropertyId(d->connector_id,
                                      DRM_MODE_OBJECT_CONNECTOR, "CRTC_ID");
  p.connector_edid = GetPropertyId(d->connector_id, DRM_MODE_OBJECT_CONNECTOR,
                                   "EDID");
  p.crtc_active = GetPropertyId(d->crtc_id, DRM_MODE_OBJECT_CRTC, "ACTIVE",
                                &crtc_active);
  d->powered = crtc_active != 0;
  p.crtc_mode_id = GetPropertyId(d->crtc_id, DRM_MODE_OBJECT_CRTC, "MODE_ID");
  p.crtc_out_fence_ptr = GetPropertyId(d->crtc_id, DRM_MODE_OBJECT_CRTC,
                                       "OUT_FENCE_PTR");
  ...
  p.plane_in_fence_fd = GetPropertyId(d->plane_id, DRM_MODE_OBJECT_PLANE,
                                       "IN_FENCE_FD");
  ...
  return p.connector_crtc_id && p.crtc_active && p.crtc_mode_id &&
         p.crtc_out_fence_ptr && p.plane_fb_id && p.plane_crtc_id &&
...
std::shared_ptr<DrmFramebuffer> DrmDevice::ImportBuffer(buffer_handle_t handle) {
  ...
  if (android::GraphicBufferMapper::get().importBufferNoValidate(handle,
                                                                 &imported) !=
      android::OK) {
  ...
  if (mapper.getPixelFormatFourCC(imported, &format) != android::OK ||
      ...
      mapper.getPlaneLayouts(imported, &layouts) != android::OK ||
      layouts.empty() || layouts.size() > kMaxPlanes || imported->numFds <= 0) {
  ...
  for (size_t i = 0; i < layouts.size(); ++i) {
    ...
    if (fd_index >= imported->numFds || imported->data[fd_index] < 0 ||
        drmPrimeFDToHandle(fd_.get(), imported->data[fd_index],
                           &fb->gem_handles_[i]) != 0) {
  ...
  const bool has_modifier = modifier != DRM_FORMAT_MOD_NONE &&
  ...
  if (has_modifier) {
    ...
    error = drmModeAddFB2WithModifiers(
        fd_.get(), width, height, format, fb->gem_handles_.data(),
        pitches.data(), offsets.data(), modifiers.data(), &fb->id_,
        DRM_MODE_FB_MODIFIERS);
  } else {
    error = drmModeAddFB2(fd_.get(), width, height, format,
                          fb->gem_handles_.data(), pitches.data(),
                          offsets.data(), &fb->id_, 0);
  }
  if (error != 0) {
  ...
  fb->width_ = width;
  fb->height_ = height;
  ALOGV("Imported FB %u %ux%u fourcc=%08x modifier=%" PRIu64, fb->id_,
        fb->width_, fb->height_, format, modifier);
  return fb;
}

bool DrmDevice::AddProperty(drmModeAtomicReq* request, uint32_t object_id,
                            uint32_t property_id, uint64_t value) const {
  return property_id != 0 &&
         drmModeAtomicAddProperty(request, object_id, property_id, value) >= 0;
}

bool DrmDevice::AtomicCommit(DrmDisplay* d,
  ...
  auto config = std::find_if(d->configs.begin(), d->configs.end(),
                             [d](const DrmConfig& c) {
                               return c.id == d->active_config;
                             });
  if (config == d->configs.end()) return false;
  uint32_t mode_blob = 0;
  if (drmModeCreatePropertyBlob(fd_.get(), &config->mode, sizeof(config->mode),
                                &mode_blob) != 0) return false;
  ...
  bool ok = request != nullptr &&
            AddProperty(request, d->connector_id, d->props.connector_crtc_id,
                        d->crtc_id) &&
  ...
  if (ok && fb != nullptr && acquire_fence >= 0 &&
      d->props.plane_in_fence_fd != 0) {
    ok = AddProperty(request, d->plane_id, d->props.plane_in_fence_fd,
                     acquire_fence);
  ...
  if (ok && drmModeAtomicCommit(fd_.get(), request, flags, nullptr) != 0) {
    ALOGE("Atomic %s failed for display %" PRId64 ": %s",
          test_only ? "test" : "present", d->id, strerror(errno));
    ok = false;
  }
...
bool DrmDevice::Test(int64_t display,
                     const std::shared_ptr<DrmFramebuffer>& fb,
                     int acquire_fence) {
...
bool DrmDevice::TestConfiguration(int64_t display) {
  ...
  if (d.powered) {
    const auto config = std::find_if(
        d.configs.begin(), d.configs.end(), [&d](const DrmConfig& item) {
          return item.id == d.active_config;
        });
    ok = ok && config != d.configs.end() &&
         drmModeCreatePropertyBlob(fd_.get(), &config->mode,
                                   sizeof(config->mode), &mode_blob) == 0 &&
         AddProperty(request, d.connector_id, d.props.connector_crtc_id,
                     d.crtc_id) &&
         AddProperty(request, d.crtc_id, d.props.crtc_mode_id, mode_blob) &&
         AddProperty(request, d.crtc_id, d.props.crtc_active, 1);
  } else {
    ok = ok && AddProperty(request, d.connector_id,
                           d.props.connector_crtc_id, 0) &&
  ...
  if (!ok) {
    ALOGE("Atomic configuration test failed for display %" PRId64 ": %s",
          display, strerror(errno));
  }
  return ok;
}

android::base::unique_fd DrmDevice::Present(
    int64_t display, const std::shared_ptr<DrmFramebuffer>& fb,
    int acquire_fence) {
  auto it = displays_.find(display);
...
bool DrmDevice::SetPower(int64_t display, bool on) {
  ...
  ok = ok && drmModeAtomicCommit(fd_.get(), request,
                                  DRM_MODE_ATOMIC_ALLOW_MODESET, nullptr) == 0;
  if (request != nullptr) drmModeAtomicFree(request);
  if (mode_blob != 0) drmModeDestroyPropertyBlob(fd_.get(), mode_blob);
  if (ok) d.powered = on;
...
std::string DrmDevice::Dump() const {
  ...
  for (const auto& [id, d] : displays_) {
    out << " display=" << id << " " << d.name << " connected=" << d.connected
        ...
        << " connector=" << d.connector_id << " crtc=" << d.crtc_id
        << " plane=" << d.plane_id << " activeConfig=" << d.active_config
